...
/// Library reference : https://pub.dev/packages/sliding_up_panel
import 'dart:math';
...
enum PanelState { OPEN, MIDDLE, CLOSED }
...
class SlidingUpPanel extends StatefulWidget {
  const SlidingUpPanel({
    ...
    this.parallaxOffset = kExtentZeroPoint1,
    this.isDraggable = true,
    this.slideDirection = SlideDirection.UP,
    this.defaultPanelState = PanelState.MIDDLE,
    this.header,
    this.footer,
  ...
  /// Flag that indicates whether or not the [backdropOpacity]
  /// follows slide or not.
  ...
  final void Function(double position, double height) onPanelSlide;
  ...
  /// If non-null, this callback is called when the panel
  /// is stop dragged. The position is a double
  /// between 0.0 and 1.0 where 0.0 is fully collapsed and 1.0 is fully open.
  final Function(double position) onFinishSliding;
  ...
  /// Adding capability to slide up using backdrop
  ///
...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  void initState() {
    ...
    _ac = AnimationController(
        ...
        value: widget.defaultPanelState == PanelState.MIDDLE
            ? (widget.snapPoint ?? kExtentZeroPoint5)
            : widget.defaultPanelState == PanelState.CLOSED
                ? 0.0
      ...
      ..addListener(() {
        ...
        if (widget.onPanelSlide != null)
          widget.onPanelSlide(_extent, _currentHeight);
      })
      ..addStatusListener((AnimationStatus status) {
        if (status == AnimationStatus.completed ||
            status == AnimationStatus.dismissed) {
          /// invoke [onFinishSliding] callback if not-null
          if (widget.onFinishSliding != null) {
            widget.onFinishSliding(_currentHeight);
          }

          if (widget.onPanelOpened != null && _extent == 1.0)
            widget.onPanelOpened();

          if (widget.onPanelClosed != null && _extent == 0.0)
            widget.onPanelClosed();

          if (widget.onPanelMiddle != null && _extent == 0.5)
            widget.onPanelMiddle();
        }
      });
    ...
    return Stack(
      ...
      children: <Widget>[
        ...
        if (widget.body != null)
          AnimatedBuilder(
            ...
            builder: (BuildContext context, Widget child) {
              return Positioned(
                top:
                    widget.parallaxEnabled ? _getParallax() : kExtentZeroPoint0,
                child: child,
              );
            },
            child: Container(
              height: AppQuery(context).height,
        ...
        if (!widget.backdropEnabled)
          ...
          GestureDetector(
            onVerticalDragEnd:
                widget.backdropTapClosesPanel || widget.backdropSlideUpEnabled
                    ? (DragEndDetails dets) {
                        ...
                        if (widget.backdropSlideUpEnabled) {
                          if ((widget.slideDirection == SlideDirection.UP
                                      ? 1
                                      : -1) *
                                  dets.velocity.pixelsPerSecond.dy <
            ...
            child: AnimatedBuilder(
                ...
                builder: (BuildContext context, _) {
                  return Container(
                    ...
                    color: widget.backdropOpacityFollowSlide
                        ? _ac.value == kExtentZeroPoint0
                            ? null
            ...
            child: AnimatedBuilder(
              ...
              child: Stack(
                children: <Widget>[
                  ...
                  // header
                  if (widget.header != null) widget.header else Container(),
  ...
  /// return extent value with 2 fraction digits
  double get _extent => double.parse(_ac.value.toStringAsFixed(2));

  double get _currentHeight => double.parse(
      (((_ac.value * (widget.maxHeight - widget.minHeight) + widget.minHeight) /
                  (AppQuery(context).height / 100)) /
              100)
          .toStringAsFixed(2));
  ...
  void _onGestureSlide(double dy) {
    ...
    if (!_scrollingEnabled ||
        _sc.position.maxScrollExtent == kExtentZeroPoint0) {
      if (widget.slideDirection == SlideDirection.UP)
    ...
    if (_isPanelOpen && _sc.hasClients && _offset <= kExtentZeroPoint0) {
      setState(() {
        if (dy < 0) {
  ...
  void _onGestureEnd(Velocity v) {
    ...
    // ignore: always_put_control_body_on_new_line
    if (_ac.isAnimating || !_slideGesture) return;
    ...
    if (v.pixelsPerSecond.dy.abs() >= minFlingVelocity) {
      ...
      if (widget.panelSnapping && widget.snapPoint != null) {
        if (_dy != null) {
          if ((_dy < 0 && _ac.value.abs() <= widget.snapPoint) ||
              _dy >= 0 && _ac.value.abs() >= widget.snapPoint)
    ...
    if (widget.panelSnapping) {
      final double _position = _ac.value.abs();
      if (widget.snapPoint != null && _dy != null) {
        if ((_dy < 0 && _position <= widget.snapPoint) ||
            _dy >= 0 && _position >= widget.snapPoint) {
          print('snap point');
          _flingPanelToPosition(widget.snapPoint, visualVelocity);
        } else if (_dy < 0 && _position > widget.snapPoint) {
          print('open');
          _open();
        } else {
          print('close');
  ...
  Future<void> _animatePanelToPosition(double value,
      {Duration duration, Curve curve = Curves.decelerate}) {
  ...
  //returns whether or not the
  //panel is at the middle
  bool get _isPanelMiddle => _ac.value == (widget.snapPoint ?? 0.5);
...
class PanelController {
  ...
  /// Returns whether or not the
  /// panel is closed.
  bool get isPanelMiddle {
    assert(isAttached, 'PanelController must be attached to a SlidingUpPanel');
    return _panelState._isPanelMiddle;
  }
